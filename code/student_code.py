...
class SimpleNet(nn.Module):
  ...
  def __init__(self, num_classes, droprate=0.5, rgb=False, verbose=False):
    ...
    self.features = nn.Sequential(
      nn.Conv2d(in_channels=in_channels, out_channels=ch1, kernel_size=9,
        stride=1, padding=0, bias=False),
      nn.MaxPool2d(kernel_size=3, stride=2, padding=0),
      nn.ReLU(),
      nn.BatchNorm2d(num_features=ch1),

      nn.Conv2d(in_channels=ch1, out_channels=ch2, kernel_size=5,
                stride=1, padding=0, bias=False),
      nn.MaxPool2d(kernel_size=3, stride=2, padding=0),
      nn.ReLU(),
      nn.BatchNorm2d(num_features=ch2),

      nn.Conv2d(in_channels=ch2, out_channels=ch3, kernel_size=4,
                stride=1, padding=0, bias=False),
      # nn.MaxPool2d(kernel_size=3, stride=2, padding=0),
      nn.LeakyReLU(),
    ...
    self.classifier = nn.Conv2d(in_channels=ch3, out_channels=num_classes,
...
def create_part2_model(model, num_classes):
  ...
  # pass
  new_linear = nn.Linear(in_features=4096, out_features=128, bias=False)
  # new_linear.weight.data.normal_(0, 1)
  ...
  new_linear = nn.Linear(in_features=128, out_features=num_classes, bias=False)
  # new_linear.weight.data.normal_(0, 1)
  if new_linear.bias is not None:
    nn.init.constant_(new_linear.bias.data, 0)
  new_classifier.append(new_linear)
...
class SketchNet(nn.Module):
  ...
  def __init__(self, num_classes, droprate=0.5, rgb=False, verbose=False):
    ...
    - verbose: (boolean) If True a hook is registered to print the size of input
      to classifier everytime the forward function is called.
    """
    super(SketchNet, self).__init__() # initialize the parent class, a must
    in_channels = 3 if rgb else 1
